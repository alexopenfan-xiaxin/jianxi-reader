...
## 3.4.0 (build 140) — 2026-06-11

### 新增
- **高频动效优化** — 首页卡片、书架卡片、底部导航、分段控制器、阅读设置面板统一使用更清晰的 `AppMotion` 动效节奏，交互反馈更自然。
- **文档打开共享元素过渡** — 从首页文档入口到阅读页标题增加 Hero 过渡，减少页面切换割裂感。
- **阅读体验动效** — 阅读进度条改为平滑缓动，Markdown 搜索定位增加当前匹配脉冲提示，文件热重载使用 fade-through 过渡。

### 调整
- **Flutter SDK 目标更新** — 项目指南中的目标 Flutter SDK 更新为 Flutter 3.44。
- **版本同步** — 应用版本更新为 `3.4.0+140`，更新检查参数同步为 `local=140`。
